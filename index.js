...
server.on('published', function(packet, client) {
  ...
  if (data[0]=="n"){
    ...
    con.query("INSERT INTO device_sessions SET ?",{device_id:data[1],
                                                   session_id:id,
       ...
       con.query("SELECT "+program+" FROM user_devices WHERE ? ",{device_id:data[1]},(error,result)=>{
       		var program_name="";
	        ...
	        con.query("SELECT pattern FROM programs WHERE ? ",{name:program_name},(error,result)=>{
			console.log(result);
			if(result.length>0){
		...
		redisClient.get("lastDose"+device_id, function (err, reply) {
                ...
                var tBegining=0;
			      ...
			      if(slope > 1000)slope =1000;
			                      ...
			                      setTimeout(function(){server.publish(message,function(){console.log("GAVE DOSE");})},50);
		      }
	        });
		}
       });
    ...
    con.query("SELECT high_intensity, normal_intensity, low_intensity, start_timeout FROM user_devices WHERE ? ",{device_id:data[1]},(error,result)=>{
	...
	var message = {
        ...
        server.publish(message,function(){console.log("GAVE SETTINGS");});
...
app.get("/",(request,response)=>{
  ...
  console.log(n)
  response.render('index', { n:n});
});
app.get("/login",(request,response)=>{
  response.render('login', { title: 'Login' });
});
app.get("/subscribe",(request,response)=>{
  response.render('subscribe', { title: 'Subscribe' });
});
app.get("/listDevices",(request,response)=>{
  response.render('listDevices', { title: 'Device List' });
});
app.get("/listPrograms",(request,response)=>{
  response.render('listPrograms', { title: 'Programs List' });
});
...
app.post('/subscribe',(req,res)=>{
  var info ={ user_name: req.body.user_name,
...
app.post('/newProgram',(req,res)=>{
	...
	con.query("INSERT INTO programs SET ?",{name:req.body.name,
                                          pattern:req.body.pattern,user_id:req.body.user_id},(error,result)=>{
    if(error){
...
app.post('/deleteDevice',(req,res)=>{
  ...
  con.query("DELETE FROM user_devices WHERE ?",{device_id:req.body.device_id},(error,result)=>{
...
app.post('/deleteSession',(req,res)=>{
  ...
  con.query("DELETE FROM device_sessions WHERE ?",{session_id:req.body.session_id},(error,result)=>{
...
app.post('/deleteProgram',(req,res)=>{
	  con.query("DELETE FROM programs WHERE name=? AND user_id=?",[req.body.name,req.body.user_id],(error,result)=>{
	   ...
	   con.query("Update user_devices SET high_intensity_program='No Program' WHERE ?",{high_intensity_program:req.body.name},(error,result)=>{

              con.query("Update user_devices SET normal_intensity_program='No Program' WHERE ?",{normal_intensity_program:req.body.name},(error,result)=>{
	      con.query("Update user_devices SET low_intensity_program='No Program' WHERE ?",{low_intensity_program:req.body.name},(error,result)=>{
	      	if(error) res.send(error);
		else res.send(result);
...
app.post('/addDevice',(req,res)=>{
 ...
 console.log(req.body);
	con.query("INSERT INTO user_devices SET ?",{user_id:req.body.user_id,
...
app.post('/newSession',(req,res)=>{
  ...
  con.query("INSERT INTO device_sessions SET ?",{device_id:req.body.device_id,
                                                 session_id:id,
                                                 session_creation_date: moment().tz("Europe/Tallinn").format('YYYY-MM-DD h:mm:ss')
                                                 },(error,result)=>{
    if(error) res.send(error);
...
app.get("/getData",(req,res)=>{
  con.query("SELECT DATE_FORMAT(date,'%Y-%m-%d %H:%i:%s') as date , rrValue as value FROM data WHERE id= ? ORDER BY logging_date ASC",{id:req.id},(error,result)=>{
      if(error) throw error;
      res.send(result);
  });
});
app.get("/getDevices",(req,res)=>{
  con.query("SELECT DATE_FORMAT(device_creation_date,'%Y-%m-%d %H:%i:%s') as date , device_name as name, device_id as id, high_intensity, normal_intensity, low_intensity, high_intensity_program, normal_intensity_program, low_intensity_program, start_timeout FROM user_devices WHERE ? ",{user_id:req.query.user_id},(error,result)=>{
      if(error) throw error;
      res.send(result);
  });
});
app.get("/getSessions",(req,res)=>{
  con.query("SELECT DATE_FORMAT(session_creation_date,'%Y-%m-%d %H:%i:%s') as date , session_id as session_id, device_id as device_id FROM device_sessions WHERE ? ORDER BY session_creation_date DESC",{device_id:req.query.device_id},(error,result)=>{
      if(error) throw error;
      res.send(result);
  });
});
app.get("/getSessionRssmdData",(req,res)=>{
	  con.query("SELECT rr_val as value FROM data WHERE session_id=? AND logging_date BETWEEN ? AND ?   ORDER BY logging_date ASC",[req.query.session_id,req.query.start_date,req.query.end_date],(error,result)=>{
		        ...
		        for(var i = 0; i < diffVecSquared.length-req.query.n; i++){
				          lnRssmd_19.push({iter:i,value:Math.log(Math.sqrt(diffVecSquared.slice(i,i+req.query.n).reduce((a, b) => a + b, 0)/(parseInt(req.query.n)-1)))*19})
				      }
		        res.send(lnRssmd_19);
		    });
});
app.get("/getSessionData",(req,res)=>{
	  con.query("SELECT DATE_FORMAT(logging_date,'%Y-%m-%d %H:%i:%s') as date , hr_val , rr_val , time_interval FROM data WHERE ?  ORDER BY logging_date ASC",{session_id:req.query.session_id},(error,result)=>{
		        if(error) throw error;
		        res.send(result);
		    });
});
